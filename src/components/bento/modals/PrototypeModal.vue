...
<template>
  <div class="max-w-7xl mx-auto">
    <div class="text-center mb-16 animate-fadeInUp">
      ...
      <h2 id="prototype-title" class="text-4xl md:text-5xl font-bold mb-4">
    ...
    <div class="grid lg:grid-cols-2 gap-8">
      <!-- Prototype Canvas -->
      <div class="bg-phantom-dark/50 backdrop-blur-sm border border-phantom-accent/20 rounded-2xl p-6">
        ...
        <!-- Canvas Area -->
        <div class="relative aspect-square bg-phantom-darker/50 rounded-xl border-2 border-phantom-accent/30 overflow-hidden">
          <div class="absolute inset-0 opacity-20" style="background-image: linear-gradient(#00f0ff 1px, transparent 1px), linear-gradient(90deg, #00f0ff 1px, transparent 1px); background-size: 20px 20px;"></div>
          <svg viewBox="0 0 600 600" class="w-full h-full">
            ...
            <line x1="40" y1="320" x2="40" y2="350" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <rect x="120" y="120" width="180" height="280" fill="none" stroke="#00f0ff" stroke-width="2" opacity="0.6" />
            ...
            <text x="360" y="100" text-anchor="middle" fill="#00f0ff" font-size="12" font-weight="bold" transform="rotate(-90 360 100)">Administración</text>
            <rect x="300" y="180" width="120" height="120" fill="none" stroke="#00f0ff" stroke-width="2" opacity="0.6" />
            <text x="360" y="235" text-anchor="middle" fill="#00f0ff" font-size="12" font-weight="bold" transform="rotate(-90 360 235)">Gerencia</text>
            <rect x="420" y="40" width="140" height="140" fill="none" stroke="#00f0ff" stroke-width="2" opacity="0.6" />
            <text x="490" y="100" text-anchor="middle" fill="#00f0ff" font-size="11" font-weight="bold" transform="rotate(-90 490 100)">Sala de Reuniones</text>
            ...
            <template v-if="cameraVisible">
              <circle cx="200" cy="70" r="10" :fill="hackedSystems.includes('cameras') ? '#22c55e' : '#ff3366'" class="animate-pulse" />
              <circle cx="450" cy="70" r="10" :fill="hackedSystems.includes('cameras') ? '#22c55e' : '#ff3366'" class="animate-pulse" />
              <circle cx="200" cy="300" r="10" :fill="hackedSystems.includes('cameras') ? '#22c55e' : '#ff3366'" class="animate-pulse" />
              <circle cx="150" cy="380" r="10" :fill="hackedSystems.includes('cameras') ? '#22c55e' : '#ff3366'" class="animate-pulse" />
              <circle cx="520" cy="520" r="10" :fill="hackedSystems.includes('cameras') ? '#22c55e' : '#ff3366'" class="animate-pulse" />
            </template>
            <line x1="300" y1="240" x2="300" y2="270" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <line x1="390" y1="40" x2="420" y2="40" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <line x1="480" y1="300" x2="510" y2="300" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <line x1="510" y1="180" x2="540" y2="180" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <line x1="255" y1="400" x2="285" y2="400" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <line x1="210" y1="450" x2="210" y2="480" :stroke="hackedSystems.includes('doors') ? '#22c55e' : '#fbbf24'" stroke-width="6" />
            <rect v-if="hackedSystems.length >= 3" x="215" y="460" width="80" height="50" fill="none" stroke="#ff3366" stroke-width="4" class="animate-pulse" />
          </svg>
          <div class="absolute bottom-4 left-4 bg-phantom-darker/90 backdrop-blur-sm rounded-lg p-3 text-xs space-y-1">
            <div class="flex items-center gap-2"><div class="w-3 h-3 bg-red-500 rounded-full"></div><span>Cámara Activa</span></div>
            <div class="flex items-center gap-2"><div class="w-3 h-3 bg-green-500 rounded-full"></div><span>Sistema Hackeado</span></div>
            <div class="flex items-center gap-2"><div class="w-6 h-3 bg-yellow-500"></div><span>Puerta Bloqueada</span></div>
            <div class="flex items-center gap-2"><div class="w-6 h-3 bg-green-500"></div><span>Puerta Desbloqueada</span></div>
          </div>
        </div>
        <button @click="cameraVisible = !cameraVisible" class="mt-4 w-full py-3 bg-phantom-darker/50 hover:bg-phantom-darker border border-phantom-accent/20 hover:border-phantom-accent/50 rounded-lg transition-all duration-300 flex items-center justify-center gap-2">
      ...
      <div class="space-y-6">
        <div class="bg-phantom-dark/50 backdrop-blur-sm border border-phantom-accent/20 rounded-2xl p-6">
          ...
          <div class="space-y-4">
            <div v-for="system in systems" :key="system.id" :class="['p-4 rounded-xl border-2 transition-all duration-300', { 'bg-green-500/10 border-green-500': hackedSystems.includes(system.id), 'bg-phantom-darker/50 border-phantom-accent/30 hover:border-phantom-accent/60': !hackedSystems.includes(system.id) }]">
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center gap-3">
                  <div :class="['w-3 h-3 rounded-full', { 'bg-green-500': hackedSystems.includes(system.id), 'bg-red-500 animate-pulse': !hackedSystems.includes(system.id) }]"></div>
                  <span class="font-semibold">{{ system.name }}</span>
                </div>
                <span class="text-sm text-gray-400 font-mono">+{{ system.time }}s</span>
              </div>
              <button @click="handleHack(system.id)" :disabled="hackedSystems.includes(system.id)" :class="['w-full py-2 rounded-lg font-medium transition-all duration-300', { 'bg-green-500/20 text-green-500 cursor-not-allowed': hackedSystems.includes(system.id), 'bg-phantom-accent text-phantom-darker hover:bg-white hover:scale-105 active:scale-95': !hackedSystems.includes(system.id) }]">
        ...
        <!-- Progress indicator -->
        <div class="bg-phantom-dark/50 backdrop-blur-sm border border-phantom-accent/20 rounded-2xl p-6">
          <h4 class="text-lg font-bold mb-4">Progreso de Infiltración</h4>
          <div class="space-y-3">
            ...
            <div class="h-3 bg-phantom-darker rounded-full overflow-hidden">
              <div class="h-full bg-gradient-to-r from-phantom-accent to-green-500 transition-all duration-500" :style="{ width: `${(hackedSystems.length / systems.length) * 100}%` }"></div>
            </div>
            <div v-if="hackedSystems.length === systems.length" class="mt-4 p-4 bg-green-500/10 border border-green-500 rounded-lg animate-fadeInUp">
              <p class="text-green-500 font-bold text-center">✓ Todos los sistemas comprometidos. El infiltrado puede avanzar.</p>
            </div>
          </div>
        </div>

        <!-- Reset button -->
        <button @click="resetSimulation" class="w-full py-3 bg-phantom-darker/50 hover:bg-red-500/20 border border-phantom-accent/20 hover:border-red-500 rounded-lg transition-all duration-300 text-gray-400 hover:text-red-500">
    ...
    <!-- Info box -->
    <div class="mt-8 bg-phantom-accent/5 border border-phantom-accent/30 rounded-2xl p-6">
      <h4 class="font-bold text-lg mb-3 text-phantom-accent">💡 Sobre este Prototipo</h4>
